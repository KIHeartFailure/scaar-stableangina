...
differences between educational level with the chi-square test. 
Continuous variables are presented with median [first quartile-third quartile] and tested for 
differences between educational level with the Kruskal-Wallis test. Standardized differences were used to quantify the differences between groups. 
...
Subgroup analyses were also performed for the fully adjusted model by lipid lowering medication, glucose lowering medication, heart failure, stroke and number of risk factors. The risk factors considered were hypertension, glucose lowering medication, lipid lowering medication, CKD and COPD. In the risk factor subgroup analyses the individual risk factor components were not included in the model. 
...
{{< include src/outcomes_subgroup.qmd >}}
...
In order to assess how robust the association between ANOCA/Comparator and the outcomes are 
to residual confounding the E-value 
was calculated [@evalue] for the model Adjusted (age, sex, region, comorbidities, medications, socioeconomics) Hazard Ratio in @tbl-outcomes assuming a rare outcome. 
The E-value is interpreted as the minimum Risk Ratio 
that an unmeasured confounder would need to have with both ANOCA/Comparator and 
the outcome, conditional on the variables currently adjusted 
for in the model, to fully eliminate the association between ANOCA/Comparator and 
outcome. 
...
{{< include src/predictors_compcase.qmd >}}

\clearpage

## Baseline characteristics - comparison with patients with ACS

{{< include src/basetab_acs.qmd >}}

\clearpage

## Association between outcomes and patients with/without ACS

{{< include src/outcomes_acs.qmd >}}
