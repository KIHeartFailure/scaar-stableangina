...
basetabfunc <- function(data, novars = NULL) {
  tab <- data %>%
    ...
    select(var, level, pmis, levels(data %>% pull(case)), p, p2)
  ...
  cn <- c("Variable", "Missing (%)", levels(data %>% pull(case)), "SD", "P-value")
  ...
  default_kable(tab,
    ...
    # add_header_above(c(" " = 1, " " = 1, "Case" = 2, " " = 1, " " = 1)) %>%
    footnote(
