...
basetabfunc_inner <- function(x, data, stratavar, catdig, contdig) {
  ...
  if (str_detect(x, "scaar_")) {
    pmis <- data %>%
      filter(!is.na(!!sym(stratavar)) & case == "Case") %>%
...
basetabfunc <- function(data, novars = NULL) {
  ...
  cn <- c("Variable", "Missing (%)", levels(data %>% pull(case)), "p-value")

  footnotetext <- c(
    "Included in adjusted regression models in the analyses comparing cases and controls.",
    "Included in imputation and regression models in the cases only analyses."
  )
...
#| cache.comments: false
#| dependson: basetabfunc
#| tbl-cap: "Baseline characteristics - Year of inclusion 2016-2019"
#| tbl-pos: "H"

basetabfunc(data = sdata %>% filter(year_cat == "2016-2019"), novars = "year_cat")
```
