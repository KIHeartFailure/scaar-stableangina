...
kmfunc <- function(data = sdata, time, event, eventname, novar = NULL,
  ...
  fuyears <- 14
  s <- summary(fit, times = seq(0, fuyears * 366, 1), extend = T)
  ...
  p <- ggplot(plot_data, aes(x = time / 365.25, y = estimate, color = strata)) +
    ...
    annotate("text",
      x = textadjx + 3.2, y = c(textadjy, textadjy - 0.1, textadjy - 0.2), label = cox_crude,
      hjust = 0.5, size = 6, fontface = "bold", colour = c("black", global_cols[2:1])
    ) +
    annotate("text",
      x = textadjx + 6.5, y = c(textadjy, textadjy - 0.1, textadjy - 0.2), label = cox_adj,
      hjust = 0.5, size = 6, fontface = "bold", colour = c("black", global_cols[2:1])
    )
  ...
  table_data <- plot_data %>%
    filter(
      time %in% c(seq(0, fuyears, 2) * 366)
    )

  t <- ggplot(table_data, aes(y = strata, x = time / 365.25)) +
    geom_text(aes(label = n.risk), size = 6, fontface = "bold", colour = rep(global_cols[2:1], each = 8)) +
    # scale_x_continuous(breaks = seq(0, endfollow, 1), limits = c(0, endfollow), expand = c(0, 0)) +
