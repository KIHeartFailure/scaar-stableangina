...
outcomesfunc_inner <- function(data, time, event, eventname, xvar = "case",
  ...
  out[3, 2] <- "Adjusted comorbidities HR (95% CI), p-value"
  ...
  out[4, 2] <- "Adjusted comorbidities, medications HR (95% CI), p-value"
  ...
  out[5, 2] <- "Adjusted comorbidities, medications, socioeconomics HR (95% CI), p-value"
  mod <- summary(coxph(
    formula(paste0(
      "Surv(", time, ",", event, "== 'Yes') ~ ", xvar, "+",
  ...
  out[6, 2] <- "Adjusted comorbidities, medications, socioeconomics with timedependent medications HR (95% CI), p-value"
  mod <- summary(coxph(
    formula(paste0(
  ...
  if (fg) {
    out[7, 2] <- "Adjusted comorbidities, medications, socioeconomics Sub-distributional HR (95% CI), p-value"
    mod <- summary(coxph(
      formula(paste0(
...
outcomesfunc <- function(var, val, novar = NULL, fgall = F) {
  ...
  default_kable(outall) %>%
    footnote(
      general_title = "",
      general = c("Abbreviations: py: person-years; HR: Hazard Ratio; CI: Confidence interval")
    ) %>%
