...
predictorsfunc <- function(time, event, xbreaks, novar = NULL, stratavar = NULL) {
  ...
  out <- tibble(
    var = mod$term,
    hr = round(exp(mod$estimate), 2),
    lci = round(exp(mod$estimate - global_z05 * mod$std.error), 2),
    uci = round(exp(mod$estimate + global_z05 * mod$std.error), 2)
  ) %>%
    mutate(
      hrprint = paste0(
        fn(hr, dig = 2),
